...
<div class="app-shell">
    ...
    <div class="main-content">
        ...
        <main class="page-body">
            ...
            {{-- KPIs INTERACTIVOS Y CLIQUEABLES --}}
            <div class="kpi-grid">
            ...
            {{-- FILTROS Y CABECERA DINÁMICA DE LA TABLA --}}
                ...
                <form method="GET" action="{{ route('dashboard.coordinador') }}" style="display:flex; align-items:center; gap:8px;">
                    ...
                    <select name="tecnico_id" onchange="this.form.submit()" style="padding:7px 12px; border-radius:9px; border:1px solid #ddd; font-size:13px; background:#fff; cursor:pointer; min-width:180px;">
                        ...
                        @foreach($tecnicos as $t)
                            <option value="{{ $t->ID_User }}" {{ request('tecnico_id') == $t->ID_User ? 'selected' : '' }}>
                                {{ $t->Usuario }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @if(request('filtro') || request('tecnico_id'))
                        <a href="{{ route('dashboard.coordinador') }}" style="font-size:13px; color:#da6714; text-decoration:none; padding:7px 12px; border:1px solid #da6714; border-radius:9px; background:#fff; font-weight:bold;">
                            ✕ Limpiar Filtros
                        </a>
                    @endif
                </form>
            ...
            {{-- Tabla General y Adaptable de Mantenimientos / Inventario --}}
            <div class="table-wrapper">
                ...
                @else
                    <table>
                        <thead>
                            @if($esFiltroEquipo ?? false)
                                {{-- Cabecera alternativa cuando miramos el inventario puro --}}
                                <tr>
                                    <th>Código Inv.</th>
                                    <th>Tipo Hardware</th>
                            ...
                            @else
                                {{-- Cabecera estándar para mantenimientos --}}
                                <tr>
                                    <th>Código Inv.</th>
                                    <th>Tipo Hardware</th>
                                    <th>Ubicación Corporativa</th>
                        ...
                        <tbody>
                            @foreach($proximos as $m)
                                @if($esFiltroEquipo ?? false)
                                    {{-- Renderizado de filas de Inventario --}}
                                    <tr>
                                ...
                                @else
                                    {{-- Renderizado de filas de Mantenimientos --}}
                                    <tr>
                                        <td><strong>N° {{ $m->Codigo_Inventario }}</strong></td>
                                        ...
                                        <td>
                                            @if(isset($m->Nombre_EstadoMantenimiento))
                                                <span class="badge badge-{{ strtolower($m->Nombre_EstadoMantenimiento) }}">
